...
$GLOBALS['TL_DCA']['tl_book'] = [
    ...
    'list'        => [
        ...
        'label' => array
        (
            'fields' => array('title', Date::parse('d.M.Y', 'endDate')),
    ...
    'fields'      => [
        ...
        'serpPreview' => [
            ...
            'eval'                    => array('url_callback'=>array('tl_book', 'getSerpUrl'), 'title_tag_callback'=>array('tl_book', 'getTitleTag'), 'titleFields'=>array('pageTitle', 'tite'), 'descriptionFields'=>array('description', 'teaser')),
            'sql'                     => null
        ],
        'teaser'     => [
            'exclude'     => true,
...
class tl_book extends Backend
{
    ...
    public function generateAlias($varValue, Contao\DataContainer $dc)
    {
        ...
        {
            return $this->Database->prepare("SELECT id FROM tl_book WHERE alias=? AND id!=?")->execute($alias, $dc->id)->numRows > 0;
        };
     ...
     * Return the SERP URL
     *
     * @param  ErdmannFreunde\BookBundle\Models\BookModel $model
     *
     * @return string
     */
    ...
    {
        $archiveModel = BookArchiveModel::findByPk($dc->activeRecord->pid);
